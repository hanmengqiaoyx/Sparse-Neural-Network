...
class Fully_Connect0(nn.Module):
    def __init__(self, in_features, out_features, bias=False):
        ...
        self.crop_gate_weights3 = Parameter(torch.Tensor(64, 64, 1, 12))
        self.crop_bn3 = nn.BatchNorm2d(64)
        self.crop_gate_weights4 = Parameter(torch.Tensor(32, 32, 1, 25))
        self.crop_bn4 = nn.BatchNorm2d(32)
        self.crop_gate_weights5 = Parameter(torch.Tensor(16, 16, 1, 50))
        self.crop_bn5 = nn.BatchNorm2d(16)
        self.crop_gate_weights6 = Parameter(torch.Tensor(8, 8, 1, 100))
        ...
        if self.training:
            if epoch % 2 == 0:
                ...
                if self.use_bias:
                    ...
                    layer60 = F.relu(self.bn0(nn.functional.conv2d(layer51, self.gate_weights0, stride=1, padding=1, bias=None)))  # [512, 32, 1]
                    layer62 = F.relu(self.crop_bn0(nn.functional.conv2d(layer60, self.crop_gate_weights0, stride=1, bias=None)))  # [512, 32, 1]
                    ...
                    layer71 = torch.cat((layer52, layer70), dim=1)  # [512, 64, 1]
                    ...
                    layer81 = torch.cat((layer42, layer80), dim=1)  # [256, 128, 1]
                    ...
                    layer91 = torch.cat((layer32, layer90), dim=1)  # [128, 256, 1]
                    ...
                    layer101 = torch.cat((layer22, layer100), dim=1)  # [64, 512, 1]
                    ...
                    layer111 = torch.cat((layer12, layer110), dim=1)  # [32, 1024, 1]
                    ...
                    layer121 = torch.cat((layer02, layer120), dim=1)  # [16, 2048, 1]
                    layer122 = F.relu(self.rc_bn5(nn.functional.conv2d(layer121, self.rc_gate_weights5, stride=1, bias=None)))  # [8, 2048, 1]
                    layer_out = F.relu(self.bn1(nn.functional.conv2d(layer122, self.gate_weights1, stride=1, bias=None)))  # [1, 2048, 1]
            ...
            if self.use_bias:
                ...
                layer70 = nn.functional.conv_transpose2d(layer62, self.up_gate_weights0, stride=(2, 1), bias=None)
                layer52 = F.relu(self.crop_bn1(nn.functional.conv2d(layer50, self.crop_gate_weights1, stride=1, bias=None)))
                layer71 = torch.cat((layer52, layer70), dim=1)  # [512, 32, 1]
                layer72 = F.relu(self.rc_bn0(nn.functional.conv2d(layer71, self.rc_gate_weights0, stride=1, bias=None)))
                layer80 = nn.functional.conv_transpose2d(layer72, self.up_gate_weights1, stride=(2, 1), bias=None)
                layer42 = F.relu(self.crop_bn2(nn.functional.conv2d(layer40, self.crop_gate_weights2, stride=1, bias=None)))
                layer81 = torch.cat((layer42, layer80), dim=1)  # [256, 64, 1]
                layer82 = F.relu(self.rc_bn1(nn.functional.conv2d(layer81, self.rc_gate_weights1, stride=1, bias=None)))
                layer90 = nn.functional.conv_transpose2d(layer82, self.up_gate_weights2, stride=(2, 1), bias=None)
                layer32 = F.relu(self.crop_bn3(nn.functional.conv2d(layer30, self.crop_gate_weights3, stride=1, bias=None)))
                layer91 = torch.cat((layer32, layer90), dim=1)  # [128, 128, 1]
                layer92 = F.relu(self.rc_bn2(nn.functional.conv2d(layer91, self.rc_gate_weights2, stride=1, bias=None)))
                layer100 = nn.functional.conv_transpose2d(layer92, self.up_gate_weights3, stride=(2, 1), bias=None)
                layer22 = F.relu(self.crop_bn4(nn.functional.conv2d(layer20, self.crop_gate_weights4, stride=1, bias=None)))
                layer101 = torch.cat((layer22, layer100), dim=1)  # [64, 256, 1]
                layer102 = F.relu(self.rc_bn3(nn.functional.conv2d(layer101, self.rc_gate_weights3, stride=1, bias=None)))
                layer110 = nn.functional.conv_transpose2d(layer102, self.up_gate_weights4, stride=(2, 1), bias=None)
                layer12 = F.relu(self.crop_bn5(nn.functional.conv2d(layer10, self.crop_gate_weights5, stride=1, bias=None)))
                layer111 = torch.cat((layer12, layer110), dim=1)  # [32, 512, 1]
                layer112 = F.relu(self.rc_bn4(nn.functional.conv2d(layer111, self.rc_gate_weights4, stride=1, bias=None)))
                layer120 = nn.functional.conv_transpose2d(layer112, self.up_gate_weights5, stride=(2, 1), bias=None)
                layer02 = F.relu(self.crop_bn6(nn.functional.conv2d(layer00, self.crop_gate_weights6, stride=1, bias=None)))
                layer121 = torch.cat((layer02, layer120), dim=1)  # [16, 1024, 1]
                layer122 = F.relu(self.rc_bn5(nn.functional.conv2d(layer121, self.rc_gate_weights5, stride=1, bias=None)))  # [8, 2048, 1]
                layer_out = F.relu(self.bn1(nn.functional.conv2d(layer122, self.gate_weights1, stride=1, bias=None)))  # [1, 2048, 1]
            gate = layer_out.view(1, self.in_features).expand(input.size(0), self.in_features)
            new_input = input.mul(gate)
