...
def model_prepare(work):
    ...
    # checkpoint = torch.load('./checkpoint/ckpt_flod1.t7')
    # net.load_state_dict(checkpoint['net'])
...
def test(epoch, dataloader, net, criterion0, vali=True):
    ...
    if vali is True:
        ...
        if acc > best_acc:
            ...
            best_acc = acc
            best_acc1 = open('Fully_Connected_Net_Sparsity.txt', 'w')
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, start_epoch+args.max_epoch):
        ...
        if lr0 < 5e-7 and lr1 < 5e-7 or epoch == 4999:
            print('Saving:')
            plt.figure(3)
            plt.subplot(2, 2, 1)
            plt.xlabel('epoch')
            ...
            plt.savefig("Fully_Connected_Net_Sparsity_First_CE.jpg")
            plt.figure(4)
            plt.subplot(2, 2, 1)
            plt.xlabel('epoch')
            plt.ylabel('train loss')
            ...
            plt.savefig("Fully_Connected_Net_Sparsity_Second_CE.jpg")
            plt.show()
            print('OVER')
            break
        else:
