...
def test(epoch, dataloader, net, criterion0, vali=True):
    ...
    if vali is True:
        ...
        if acc > best_acc:
            ...
            best_acc = acc
            best_acc1 = open('ResNet18_Sparsity.txt', 'w')
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, start_epoch+args.max_epoch):
        ...
        if epoch == 400:
            ...
            plt.savefig("ResNet18_Sparsity_First_CE_400.jpg")
            plt.figure(2)
            plt.subplot(2, 2, 1)
            plt.xlabel('epoch')
            ...
            plt.savefig("ResNet18_Sparsity_Second_CE_400.jpg")
            ...
            plt.ylabel('test acc')
            plt.plot([i for i in range(epoch0)], test_acc_array0, '-')
            plt.savefig("ResNet18_Sparsity_First_CE.jpg")
            plt.figure(4)
            plt.subplot(2, 2, 1)
            plt.xlabel('epoch')
            plt.ylabel('train loss')
            ...
            plt.ylabel('test acc')
            plt.plot([i for i in range(epoch1)], test_acc_array1, '-')
            plt.savefig("ResNet18_Sparsity_Second_CE.jpg")
            plt.show()
            print('OVER')
            break
        else:
